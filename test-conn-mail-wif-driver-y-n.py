...
def get_wifi_details():
    wifi_details = subprocess.check_output(["netsh", "wlan", "show", "interfaces"], text=True)
    ssid = wifi_details.split("SSID                   : ")[1].splitlines()[0].strip()
    bssid = wifi_details.split("BSSID                  : ")[1].splitlines()[0].strip()
    signal = wifi_details.split("Signal                 : ")[1].splitlines()[0].strip()
    radio_type = wifi_details.split("Radio type             : ")[1].splitlines()[0].strip()
    channel = wifi_details.split("Channel                : ")[1].splitlines()[0].strip()
    security = wifi_details.split("Authentication         : ")[1].splitlines()[0].strip()
...
commands = [
    ...
    "driverquery | findstr \"PCI Wireless zapprd Realtek\"",
...
# Prompt user for sending the report
send_report = input("Please type Yes to send the report to Network@ust.com, else type No: ")
if send_report.lower() == "yes":
    # Send the email using Outlook
    outlook = win32.Dispatch("Outlook.Application")
    mail = outlook.CreateItem(0)  # 0 represents a new mail item
    mail.To = "praveen.sreedharan@ust.com"
    mail.Subject = "Initial Connectivity Test Data"
    mail.Body = message
    mail.Send()
    print("Email sent successfully.")
else:
    print("Report not sent. Thank you!")
